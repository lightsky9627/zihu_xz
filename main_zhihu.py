...
class ZhihuParser:
    ...
    def check_connect_error(self, target_link):
        ...
        # Save raw HTML for regex search
    ...
    def extract_font_data(self):
        """
        Extract the custom font data from the page style.
        ADOPTED LOGIC: Use specific regex on full HTML and pick the 3rd match if available.
        """
        ...
        # Note: The user's regex is specific to ttf. We should probably allow woff/woff2 too just in case,
        # or stick strictly to their logic. Let's start strictly as they said it works.
        # However, Python re module needs proper escaping.
        
        pattern = r"@font-face\s*\{[^\}]*?src:\s*url\((?:data:font/(?:ttf|woff|woff2);charset=utf-8;base64,)?([A-Za-z0-9+/=]+)\)"
        # Simplified regex to catch base64 strings in font-face src
        # The user's regex: r"@font-face\s*\{[^\}]*?src:\s*url\(data:font/ttf;charset=utf-8;base64,([A-Za-z0-9+/=]+)\)"
        
        matches = re.findall(r"@font-face\s*\{[^\}]*?src:\s*url\(data:font/[^;]+;charset=utf-8;base64,([A-Za-z0-9+/=]+)\)", self.html_content)
        
        if matches:
            print(f"DEBUG: Found {len(matches)} base64 fonts.")
            # User logic: if matches and len(matches) > 2: base64_font_data = matches[2]
            target_index = -1
            if len(matches) > 2:
                target_index = 2
                print("DEBUG: Selecting the 3rd font (index 2) as per user logic.")
            elif len(matches) > 0:
                # Fallback if fewer than 3? Maybe the last one? 
                # Let's try to match user behavior: "matches[2]" implies strict index.
                # If we don't have 3, we might fail or pick the last one.
                # Let's pick the last one as a fallback.
                target_index = -1
                print("DEBUG: Fewer than 3 fonts found. Selecting the last one.")
            
            if target_index is not None:
                try:
                    b64_str = matches[target_index]
                    font_bytes = base64.b64decode(b64_str)
                    font_items.append({'type': 'content', 'data': font_bytes})
                except Exception as e:
                    print(f"Failed to decode base64 font: {e}")
    ...
    def save_and_transform(self, title_element, content_element, author, target_link, date=None):
        ...
        font_items = self.extract_font_data()
        if font_items:
            print(f"Found {len(font_items)} custom fonts.")
            self.log('info', f"Found {len(font_items)} custom fonts.")
            ...
            for item in font_items:
                try:
                    if item['type'] == 'url':
                        ...
                        self.log('info', f"Downloading font: {font_url}")
                        font_response = self.session.get(font_url)
                        font_response.raise_for_status()
                        self.font_decoder.load_font(font_response.content)
                    elif item['type'] == 'content':
                        print("Loading embedded base64 font...")
                        self.log('info', "Loading embedded base64 font...")
                        self.font_decoder.load_font(item['data'])
                        
                    print("Font loaded successfully.")
                except Exception as e:
                    print(f"Failed to load custom font: {e}")
                    self.log('error', f"Failed to load custom font: {e}")
        else:
            print("No custom fonts found in this article.")
        ...
        if content_element is not None:
            ...
            for img in content_element.find_all("img"):
                try:
                    ...
                    # URL Encode the path for Markdown compatibility (handling parenthesis, Chinese chars, etc.)
                    # safe='/' ensures we don't encode the directory separator
                    img["src"] = quote(img_path, safe='/')

                    # 下载图片并保存到本地 (Save uses the original file system path, not the encoded one)
                    # Note: img_path here is the relative path "Folder/Image.png". 
                    # We need to ensure we are saving to the correct local path.
            ...
            # Decode obfuscated font characters
            content = self.font_decoder.decode(content)
            
            # ADOPTED LOGIC: Post-processing replacements
            # 1. Replace '广...上' with '「...」'
            content = re.sub(r'广(.*?)上', r'「\1」', content)
            
            # 2. Replace 'o' with '。' (Common OCR error/obfuscation)
...
if __name__ == "__main__":
    ...
    url = "https://www.zhihu.com/question/343632603/answer/1452954680"
